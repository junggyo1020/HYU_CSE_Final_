...
def main(indir, outdir, varDict, gui = False, source_text = False):
	...
	if indir == "":
		...
		#Revised 6-21-17
		# JW : 텍스트의 동의어를 인식하여 어휘적 다양성을 평가할 수 있도록 도와주는 라이브러리
		import nltk
		...
		def wordnet_dict_build(target_list, syn_dict):
			...
			# 기존의 wordnet_dict_build 함수에 NLTK WordNet을 활용하여 동의어 사전을 구축하도록 수정.
			# :param target_list: 분석할 단어 리스트
			# :param syn_dict: 기존에 사용된 동의어 사전 딕셔너리
			# :return: target_syn_dict - 텍스트의 각 문장/단락에 대한 동의어 리스트를 포함하는 사전

			counter = len(target_list)  # 텍스트의 문장/단락 수
			...
			for i in range(counter):
				if len(target_list[i]) < 1:
					...
					syn_list1 = []
					for item in target_list[i]:  # 각 단어에 대해 처리
						if item in syn_dict:
							syns = syn_dict[item]
						else:
							# Use NLTK WordNet to find synonyms
							syns = {item}  # 기본적으로 자기 자신을 포함
							for syn in wordnet.synsets(item):
								for lemma in syn.lemmas():
									syns.add(lemma.name())

							syns = list(syns)  # 중복 제거 후 리스트로 변환
							syn_dict[item] = syns  # 새롭게 찾은 동의어를 사전에 추가

						# 동의어 목록을 syn_list1에 추가
						syn_list1.extend(syns)
		...
		def ngram_pos_dict_spacy(text,lemma = False, syn_dict={}): #updated 2023-08-05
			# if lemma == "no":
			# 	token_get = 0
			# if lemma == "yes":
			# 	token_get = 1
		...
		# JW : 텍스트 내 동의어의 사용 빈도와 고유한 동의어의 수를 추적하는 SDI 계산 함수
		def synonym_diversity_index(header_list, index_list, name_suffix, word_list, syn_dict):
			# 동사/명사 리스트가 비어있는 경우 0으로 초기화
			if len(word_list) < 1:
				sdi_value = 0
			else:
				# 사용된 동의어를 기록하는 집합 (중복을 허용하지 않기 위해 set 사용)
				used_synonyms = set()
				...
				for word in word_list:
					...
					if word in syn_dict:
						...
						# 동의어 목록에서 사용된 단어들을 집합에 추가
						for synonym in syn_list:
							used_synonyms.add(synonym)

				# SDI 계산: 고유하게 사용된 동의어의 수 / 사용된 전체 동의어 집합의 크기
				total_synonym_sets = sum(len(syn_dict[word]) for word in word_list if word in syn_dict)
				unique_synonyms_used = len(used_synonyms)
				sdi_value = unique_synonyms_used / total_synonym_sets if total_synonym_sets > 0 else 0

			# 지표에 추가
			header_list.append("sdi_" + name_suffix)
			index_list.append(sdi_value)
		...
		punctuation = "' . , ? ! ) ( % / - _ -LRB- -RRB- SYM : ; _SP".split(" ") #added "_SP", which is the spacy tag for extra spaces (including \xa0)
		punctuation.append('"')
		...
		###BEGIN PARAGRAPH OVERLAP SECTION###
			...
			if varDict["overlapSynonym"] == True:
				...
				# JW : SDI 관련 지표 추가
				noun_sdi = synonym_diversity_index(header_list, index_list, "noun", noun_text, wn_noun_dict)
				verb_sdi = synonym_diversity_index(header_list, index_list, "verb", verb_text, wn_verb_dict)
